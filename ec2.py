...
def create_security_group(vpc_id, region_name):
    ...
    sec_group = ec2.create_security_group(
        GroupName='grafana_group', Description='grafana_group', VpcId=vpc_id)
...
def get_security_group_id(region_name):
    ...
    response=client.describe_security_groups(
        Filters=[
            {'Name': 'group-name', 'Values': ['grafana_group']}
        ],
    )
...
def launch_app(host_id, key_name, github_repo):
    ...
    stdin, stdout, stderr = client.exec_command('sudo apt install git -y && git clone '+github_repo)
    print (stdout.readlines())
    time.sleep(3)
